...
import { Button, Card, CardBody, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Progress, Tab, Tabs, useDisclosure } from "@nextui-org/react";
...
export const MateriBelajarDesktop = ({ setActiveVideo }) => {
  ...
  const progres =
    dataDetail && (dataDetail.chapter.map((chapter) => chapter.lessons.reduce((total, lesson) => (lesson.is_done ? total + 1 : total), 0)).reduce((total, previewCount) => total + previewCount, 0) / (dataDetail.total_lesson || 1)) * 100;
        ...
        <div className="flex h-full  justify-center content-center items-center ">
          <div className=" space-y-4 w-full mx-6 ">
            ...
            <div className="flex justify-between items-center gap-2">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" fill="#45C440" height="32" width="32" viewBox="0 0 512 512">
                <path d="M256 48a208 208 0 1 1 0 416 208 208 0 1 1 0-416zm0 464A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-111 111-47-47c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9l64 64c9.4 9.4 24.6 9.4 33.9 0L369 209z" />
              </svg>
              <p className="font-medium text-[#116E63]">{progres}%</p>
              <Progress aria-label="Loading..." value={progres} className="max-w-md" />
            </div>
            <div className="mx-2">
              {dataDetail &&
                dataDetail.chapter.map((courses, indeks) => (
                  <div key={courses.id}>
                    <div className="flex justify-between items-center text-sm font-semibold ">
                      ...
                      <p className=" text-[#F2A227] ">{courses.lessons.reduce((total, lesson) => total + lesson.duration, 0)} Menit</p>
                    </div>
                    <div className="space-y-2 my-2">
                      {courses.lessons.map((lesson, lessonIndex) => {
                        const cumulativeLessonIndex = dataDetail.chapter.slice(0, indeks).reduce((total, ch) => total + ch.lessons.length, 0) + lessonIndex + 1;

                        return (
                          <div key={lesson.id} className="flex gap-2 justify-between items-center text-sm font-medium">
                            <p className="bg-[#EBF3FC] px-4 py-2 rounded-full">{cumulativeLessonIndex}</p>
                            <p className="w-full flex justify-start ">{lesson.name}</p>
                            {lesson.is_preview ? (
                              <button
                              ...
                              >
                                <svg xmlns="http://www.w3.org/2000/svg" fill={lesson.is_done ? "#45C440" : "#116E63"} height="20" width="20" viewBox="0 0 512 512">
            ...
            {dataDetail.is_buy ? (
              ...
              <div className="flex justify-center ">
                <Button className="bg-[#116E63] text-white w-full font-semibold desktop:w-[23rem] mt-4 h-[2.5rem] rounded-xl hover:opacity-70" onPress={onOpen}>
                  Beli Sekarang
                </Button>
                <Modal className="relative" isOpen={isOpen} onOpenChange={onOpenChange}>
                      ...
                      <>
                        ...
                        <ModalFooter className="flex justify-center">
                          <Button
                            ...
                            onClick={() => {
                              navigate(`/detailKelasPembayaran/${dataDetail.id}`);
                            }}
                          >
                            <p>Beli Sekarang</p>
