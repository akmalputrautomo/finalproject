...
export const DetailKelas = () => {
  ...
  const totalNilai = dataDetail && dataDetail.chapter[0] && dataDetail.chapter[0].lesson.reduce((total, objek) => total + objek.duration, 0);
  const totalNilai2 = dataDetail && dataDetail.chapter[1] && dataDetail.chapter[1].lesson.reduce((total, objek) => total + objek.duration, 0);
  ...
  const defaultVideo = dataDetail && dataDetail.chapter[0] && dataDetail.chapter[0].lesson[0].video;

  console.log(dataDetail && dataDetail.chapter[0] && dataDetail.chapter[0].lesson[0].is_done, "done");
                                  ...
                                  <div className=" space-y-4 w-full mx-6 ">
                                    ...
                                    <div className="mx-4">
                                      <div className="flex justify-between items-center text-sm font-semibold ">
                                        <p className="text-[#116E63]">Chapter 1 - {dataDetail && dataDetail.chapter && dataDetail.chapter[0] && dataDetail.chapter[0].name}</p>
                                        <p className=" text-[#F2A227] ">{totalNilai} Menit</p>
                                      </div>
                                      {dataDetail &&
                                        dataDetail.chapter[0].lesson.map((courses, indeks) => (
                                          <div className="mb-2" key={[]}>
                                            <div className="flex gap-2 justify-between items-center text-sm font-medium">
                                              <p className="bg-[#EBF3FC] px-4 py-2 rounded-full">{indeks + 1}</p>
                                              <p className="w-full flex justify-start">{courses.name}</p>
                                              {/* Play Video */}
                                              <button onClick={() => changeVideo(courses.video)} disabled={courses.is_done}>
                                                <svg xmlns="http://www.w3.org/2000/svg" fill={is_done ? "red" : "#45C440"} height="20" width="20" viewBox="0 0 512 512">
                                      ...
                                      <div className="flex justify-between items-center text-sm font-semibold  ">
                                        <p className="text-[#116E63]">Chapter 2 - {dataDetail && dataDetail.chapter && dataDetail.chapter[1] && dataDetail.chapter[1].name}</p>
                                        <p className=" text-[#F2A227] ">{totalNilai2} Menit</p>
                                      </div>
                                      {dataDetail &&
                                        dataDetail.chapter[1].lesson.map((courses, indeks) => (
                                          <div className="mb-2" key={[]}>
                                            <div className="flex gap-2 justify-between items-center text-sm font-medium">
                                              <p className="bg-[#EBF3FC] px-4 py-2 rounded-full">{dataDetail.chapter[0].lesson.length + indeks + 1}</p>
                                              <p className="w-full flex justify-start">{courses.name}</p>
                                              {/* Play Video */}
